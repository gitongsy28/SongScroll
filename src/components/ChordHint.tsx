...
import { ChordVoicing, getGuitarChordData, resolveChordVoicing } from '../utils/guitarChords';
...
/**
 * Reusable white-card Chord Diagram Graphic.
 * Exactly matches ChordHint dimensions, typography, and styling.
 */
export const ChordDiagramCard: React.FC<ChordDiagramCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white text-black rounded-lg border border-slate-300 p-1 shadow-sm flex flex-col items-center select-none ${className}`}
    >
      {voicing ? (
        <svg
          width={svgWidth}
        ...
        <div className="w-20 py-2 text-center">
          ...
          <span className="text-[9px] text-slate-500 block mt-0.5">Defined Chord</span>
        </div>
      )}
    </div>
  );
 ...
 * Always respects custom defined chord voicings to match Detailed Mode consistently.
 */
  ...
  return (
    ...
    >
      <div className="relative">
        {/* Directional Caret */}
        {placement === 'top' ? (
          <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-white border-r border-b border-slate-300 rotate-45 z-10" />
        ) : (
          <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-white border-l border-t border-slate-300 rotate-45 z-10" />
        )}

        <ChordDiagramCard chordName={chordName} voicing={voicing} className="shadow-xl" />
      </div>
    </div>
  );
};
